...
def parse_config_into_itc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Itecoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/Itecoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/Itecoin_config.h'), "w", encoding="utf8") as itc_config:
        itc_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Itecoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_itc_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/Itecoin_config.h'), os.path.join(SOURCE_DIR, 'config/Itecoin-config.h'))
