...
"""Test Itecoind with different proxy configuration.

Test plan:
- Start Itecoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on Itecoind side:
...
class ProxyTest(ItecoinTestFramework):
    ...
    def node_test(self, node, *, proxies, auth, test_onion, test_cjdns):
        ...
        # Note: Itecoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
