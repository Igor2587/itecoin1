...
class MempoolLimitTest(ItecoinTestFramework):
    ...
    def test_mid_package_eviction(self):
        ...
        # Specific number of Igors to fit within a small window. The parent_cpfp + child package needs to be
        ...
        magic_Igors = 1200
        cpfp_Igors = int(cpfp_fee * COIN) + magic_Igors

        child = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_utxos, fee_per_output=cpfp_Igors)
    ...
    def run_test(self):
        ...
        worst_feerate_itcvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_itcvb = min(worst_feerate_itcvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_fee = (worst_feerate_itcvb) * (target_weight_each // 4) - Decimal("0.00001")
        ...
        assert_greater_than(worst_feerate_itcvb, (parent_fee + child_fee) / (tx_parent_just_below["tx"].get_vsize() + tx_child_just_above["tx"].get_vsize()))
