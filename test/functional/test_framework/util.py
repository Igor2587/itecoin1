...
def get_fee(tx_size, feerate_itc_kvb):
    """Calculate the fee in itc given a feerate is itc/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_itc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
...
def wait_until_helper_internal(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `ItecoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "Itecoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> Tuple[dict, pathlib.Path]:
    ...
    if sys.platform == "win32":
        ...
        datadir = temp_dir / "Itecoin"
    else:
        env = dict(HOME=str(temp_dir))
        if sys.platform == "darwin":
            datadir = temp_dir / "Library/Application Support/Itecoin"
        else:
            datadir = temp_dir / ".Itecoin"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "Itecoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "Itecoin.conf")):
        with open(os.path.join(datadir, "Itecoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
