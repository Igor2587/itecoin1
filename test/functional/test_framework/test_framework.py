#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Itecoin Core developers
...
TMPDIR_PREFIX = "Itecoin_func_test_"
...
class ItecoinTestMetaClass(type):
    """Metaclass for ItecoinTestFramework.

    Ensures that any attempt to register a subclass of `ItecoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ItecoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ItecoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ItecoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ItecoinTestFramework(metaclass=ItecoinTestMetaClass):
    """Base class for a Itecoin test script.

    Individual Itecoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave Itecoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop Itecoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use Itecoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "Itecoind": ("Itecoind", "ItecoinD"),
            "Itecoin-cli": ("Itecoincli", "ItecoinCLI"),
            "Itecoin-util": ("Itecoinutil", "ItecoinUTIL"),
            "Itecoin-wallet": ("Itecoinwallet", "ItecoinWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: Itecoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("ItecoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'Itecoind', self.options.Itecoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'Itecoin-cli', self.options.Itecoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a Itecoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple Itecoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as Itecoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    Itecoind=self.options.Itecoind,
                    Itecoin_cli=self.options.Itecoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in Itecoin.conf
    ...
    def skip_if_no_Itecoind_tracepoints(self):
        """Skip the running test if Itecoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("Itecoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_Itecoind_zmq(self):
        """Skip the running test if Itecoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("Itecoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if Itecoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("Itecoin-wallet has not been compiled")

    def skip_if_no_Itecoin_util(self):
        """Skip the running test if Itecoin-util has not been compiled."""
        if not self.is_Itecoin_util_compiled():
            raise SkipTest("Itecoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if Itecoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("Itecoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether Itecoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether Itecoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_Itecoin_util_compiled(self):
        """Checks whether Itecoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_Itecoin_UTIL")
