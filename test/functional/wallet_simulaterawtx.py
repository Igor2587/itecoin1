...
class SimulateTxTest(ItecoinTestFramework):
    ...
    def run_test(self):
        ...
        Itecoin_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 itc decrease, w2 sees + 5 itc
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + Itecoin_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - Itecoin_fee + 4.9999       -5 - Itecoin_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - Itecoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - Itecoin_fee)
        ...
        tx2 = funding["hex"]
        Itecoin_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + Itecoin_fee2))
