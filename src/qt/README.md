This directory contains the source code for the Itecoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Itecoin UI.
...
- ClientModel is used by the main application `Itecoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a Itecoin:... link from a browser).
...
* UI elements like ItecoinAmountField, which inherit from QWidget.
* `Itecoinstrings.cpp`: automatically generated
* `Itecoinunits.(h/cpp)`: itc / mitc / etc. handling
* `callback.h`
...
**Note:** Do not change `local/Itecoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `Itecoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `Itecoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `Itecoin-qt`
9. You're all set! Start developing, building, and debugging the Itecoin Core GUI
