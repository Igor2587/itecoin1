...
const std::string ItecoinGUI::DEFAULT_UIPLATFORM =
...
ItecoinGUI::ItecoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &ItecoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &ItecoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &ItecoinGUI::showModalOverlay);
...
void ItecoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Itecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and Itecoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &ItecoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &ItecoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &ItecoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Itecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Itecoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Itecoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Itecoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a Itecoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Itecoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &ItecoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &ItecoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &ItecoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &ItecoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &ItecoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &ItecoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &ItecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &ItecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &ItecoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &ItecoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &ItecoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &ItecoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &ItecoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &ItecoinGUI::showDebugWindow);
}

void ItecoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void ItecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &ItecoinGUI::setCurrentWalletBySelectorIndex);
...
void ItecoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &ItecoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &ItecoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &ItecoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &ItecoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void ItecoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void ItecoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &ItecoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &ItecoinGUI::removeWallet);
...
WalletController* ItecoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void ItecoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &ItecoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &ItecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &ItecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &ItecoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &ItecoinGUI::incomingTransaction);
    connect(this, &ItecoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void ItecoinGUI::removeWallet(WalletModel* walletModel)
{
...
void ItecoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void ItecoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void ItecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ItecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void ItecoinGUI::createTrayIcon()
{
...
void ItecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &ItecoinGUI::toggleHidden);
...
void ItecoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void ItecoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void ItecoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void ItecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void ItecoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void ItecoinGUI::openClicked()
{
...
void ItecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ItecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ItecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ItecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ItecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ItecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void ItecoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void ItecoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Itecoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void ItecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ItecoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void ItecoinGUI::updateHeadersSyncProgressLabel()
{
...
void ItecoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void ItecoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &ItecoinGUI::quitRequested);
...
void ItecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void ItecoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &ItecoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void ItecoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void ItecoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &ItecoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &ItecoinGUI::show);
                e->ignore();
            }
        }
    }
...
void ItecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ItecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ItecoinGUI::incomingTransaction(const QString& date, ItecoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ItecoinUnits::formatWithUnit(unit, amount, true));
...
void ItecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ItecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ItecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ItecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ItecoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void ItecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ItecoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void ItecoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void ItecoinGUI::updateWindowTitle()
{
...
void ItecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ItecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ItecoinGUI::detectShutdown()
{
...
void ItecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void ItecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ItecoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = ItecoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void ItecoinGUI::subscribeToCoreSignals()
{
...
void ItecoinGUI::unsubscribeFromCoreSignals()
{
...
bool ItecoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<ItecoinUnit> units = ItecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ItecoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, ItecoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const ItecoinUnit u : ItecoinUnits::availableUnits()) {
        menu->addAction(ItecoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(ItecoinUnit newUnits)
{
    setText(ItecoinUnits::longName(newUnits));
}
