...
/** Call node.updateRwSetting() with Itecoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Itecoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/Itecoin/Itecoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because Itecoin 22.x will not try to read these.
...
//! Convert enabled/size values to Itecoin -prune setting.
...
//! Get pruning enabled value to show in GUI from Itecoin -prune setting.
...
//! Get pruning size value to show in GUI from Itecoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayItecoinUnit")) {
        settings.setValue("DisplayItecoinUnit", QVariant::fromValue(ItecoinUnit::itc));
    }
    QVariant unit = settings.value("DisplayItecoinUnit");
    if (unit.canConvert<ItecoinUnit>()) {
        m_display_Itecoin_unit = unit.value<ItecoinUnit>();
    } else {
        m_display_Itecoin_unit = ItecoinUnit::itc;
        settings.setValue("DisplayItecoinUnit", QVariant::fromValue(m_display_Itecoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if Itecoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_Itecoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ItecoinUnit>() == m_display_Itecoin_unit) return;
    m_display_Itecoin_unit = new_unit.value<ItecoinUnit>();
    QSettings settings;
    settings.setValue("DisplayItecoinUnit", QVariant::fromValue(m_display_Itecoin_unit));
    Q_EMIT displayUnitChanged(m_display_Itecoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/Itecoin/Itecoin/pull/8273
    ...
    // (https://github.com/Itecoin-core/gui/issues/567).
