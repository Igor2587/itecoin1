...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<ItecoinUnit>("ItecoinUnit");
#else
    qRegisterMetaType<ItecoinUnit>("ItecoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. Itecoin_de.qm (shortcut "de" needs to be defined in Itecoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. Itecoin_de_DE.qm (shortcut "de_DE" needs to be defined in Itecoin.qrc)
...
static const char* qt_argv = "Itecoin-qt";

ItecoinApplication::ItecoinApplication()
...
void ItecoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ItecoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ItecoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ItecoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ItecoinApplication::createOptionsModel(bool resetSettings)
{
...
void ItecoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ItecoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ItecoinGUI::quitRequested, this, &ItecoinApplication::requestShutdown);
...
void ItecoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ItecoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ItecoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ItecoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ItecoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ItecoinApplication::handleRunawayException);
    connect(this, &ItecoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ItecoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ItecoinApplication::parameterSetup()
{
...
void ItecoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ItecoinApplication::requestInitialize()
{
...
void ItecoinApplication::requestShutdown()
{
...
void ItecoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // Itecoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ItecoinGUI::handlePaymentRequest);
            connect(window, &ItecoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ItecoinApplication::handleRunawayException(const QString &message)
{
...
void ItecoinApplication::handleNonFatalException(const QString& message)
{
...
WId ItecoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ItecoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ItecoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(Itecoin);
    Q_INIT_RESOURCE(Itecoin_locale);
    ...
    /// 6-7. Parse Itecoin.conf, determine network, switch to network specific
    ...
    // Itecoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
