...
const char * const Itecoin_CONF_FILENAME = "Itecoin.conf";
const char * const Itecoin_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //Itecoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", Itecoin_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Itecoin
    // macOS: ~/Library/Application Support/Itecoin
    // Unix-like: ~/.Itecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Itecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Itecoin";
#else
    // Unix-like
    return pathRet / ".Itecoin";
#endif
#endif
}
