...
"""Script for verifying Itecoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from Itecoincore.org and Itecoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/Itecoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://Itecoincore.org"
HOST2 = "https://Itecoin.org"
VERSIONPREFIX = "Itecoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Itecoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"Itecoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by Itecoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the Itecoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes Itecoin.org lags behind Itecoincore.org.)')
    )
