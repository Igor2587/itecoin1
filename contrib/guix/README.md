# Bootstrappable Itecoin Core Builds

This directory contains the files necessary to perform bootstrappable Itecoin
Core builds.
...
To build Itecoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Itecoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/Itecoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/Itecoin-core/guix.sigs).
