...
Tracing (USDT) support for Itecoin Core on Linux using. For more information on
USDT support in Itecoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `Itecoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `Itecoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Itecoin Core USDT documentation] on how to list available tracepoints in your
`Itecoind` binary.

[Itecoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/Itecoind
```
...
Itecoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/Itecoind
```
...
In a different terminal, starting Itecoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/Itecoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/Itecoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/Itecoind
```
