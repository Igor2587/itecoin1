...
""" Example logging Itecoin Core mempool events using the mempool:added,
...
def main(Itecoind_path):
    Itecoind_with_usdts = USDT(path=str(Itecoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    Itecoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    Itecoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    Itecoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    Itecoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[Itecoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/Itecoind")
        exit(1)
