...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing Itecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='Itecoin-cli', help='Itecoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Itecoin address to which the faucet should send')
...
parser.add_argument('Itecoin_cli_args', nargs='*', help='Arguments to pass on to Itecoin-cli (default: -signet)')

args = parser.parse_args()

if args.Itecoin_cli_args == []:
    args.Itecoin_cli_args = ['-signet']


def Itecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.Itecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = Itecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = Itecoin_cli(['getnewaddress', 'faucet', 'bech32'])
