# User-space, Statically Defined Tracing (USDT) for Itecoin Core

Itecoin Core includes statically defined tracepoints to allow for more
...
**Note**: Itecoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Itecoin Core
...
Multiple tools can list the available tracepoints in a `Itecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Itecoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/Itecoind
stap net        outbound_message 0x0000000000107c05 /src/Itecoind
stap validation block_connected  0x00000000002fb10c /src/Itecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Itecoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/Itecoind | grep NT_STAPSDT -A 4 -B 2
