...
Itecoin Core version 0.20.0 is now available from:

  <https://Itecoincore.org/bin/Itecoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `Itecoin_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Itecoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Itecoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Itecoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Itecoin Core. Itecoin Core logs debug messages
...
- OpenSSL is no longer used by Itecoin Core.  (#17265)

- BIP70 support has been fully removed from Itecoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/Itecoin/Itecoin/issues/16599),
  [PR #16702](https://github.com/Itecoin/Itecoin/pull/16702), and the
  `Itecoind help` for more information.  This option is experimental and
...
- All custom settings configured when Itecoin Core starts are now
...
- The "Start Itecoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://Itecoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Itecoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Itecoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/Itecoin-core/HWI) for
  signing.  Future versions of Itecoin Core should support a GUI option
...
- Itecoin Core's automatically-generated source code documentation is
  now available at https://doxygen.Itecoincore.org.  (#17596)
...
- The `Itecoin-cli` utility used with the `-getinfo` parameter now
...
- The `Itecoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `Itecoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_Itecoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBItecoin_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_Itecoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBItecoin_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for Itecoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate Itecoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest Itecoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_Itecoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (Itecoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone Itecoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Itecoin Core files (hebasto)
...
- #17119 Fix broken Itecoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from Itecoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.Itecoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to Itecoin-wallet (hel-o)
...
- #16115 On Itecoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from Itecoin-cli -getinfo (malevolent)
