Itecoin Core version 0.19.0.1 is now available from:

  <https://Itecoincore.org/bin/Itecoin-core-0.19.0.1/>
...
Itecoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Itecoin Core on unsupported systems.
...
to open the Itecoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Itecoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/Itecoin/Itecoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Itecoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Itecoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `Itecoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Itecoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Itecoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Itecoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Itecoin Core,
  ...
  This prevents Itecoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Itecoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Itecoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `Itecoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Itecoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Itecoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Itecoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Itecoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  ...
  vbytes, not Igor per vbyte. (#16257)
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Itecoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Itecoin AEAD (jonasschnelli)
...
- #15639 Itecoin-wallet tool: Drop libItecoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in Itecoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "Itecoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of Itecoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from Itecoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_Itecoin to auto generated project files (sipsorcery)
...
- #15773 Add ItecoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_Itecoin output (practicalswift)
...
- #15896 feature_filelock, interface_Itecoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Itecoin Core (luke-jr)
...
- #16277 Suppress output in test_Itecoin for expected errors (gertjaap)
...
- #16489 log: harmonize Itecoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (ItecoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example Itecoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBItecoin_SERVER (Bushstar)
...
- #16080 build/doc: update Itecoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in Itecoin.conf (fanquake)
...
- #16621 add default Itecoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the Itecoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, Itecoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/Itecoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/Itecoin/Itecoin/).
