Itecoin Core version 0.11.0 is now available from:

  <https://Itecoin.org/bin/Itecoin-core-0.11.0/>
...
For example, add the following to `Itecoin.conf`:
...
related to the blockchain in the Itecoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Itecoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Itecoin Core will 
...
For an example script see [Itecoin-submittx](https://github.com/laanwj/Itecoin-submittx).
...
- #5334 `cf87536` libItecoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new Itecoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ItecoinAmountField
- #5753 `0f44672` Add Itecoin logo to about screen
...
- #5809 `46bfbe7` Add Itecoin-cli man page
...
- #5849 `41259ca` contrib/init/Itecoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Itecoin-Qt.app to Itecoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for Itecoin-tx)
