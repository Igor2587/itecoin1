...
The Itecoin Core daemon can be configured to act as a trusted "border
router", implementing the Itecoin wire protocol and relay, making
...
The ZeroMQ feature in Itecoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building Itecoind:
...
For instance:
    ...
    $ Itecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=ipc:///tmp/Itecoind.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in Itecoin.conf.
...
From the perspective of Itecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into Itecoind directly. Furthermore, no information is
...
using. Itecoind appends an up-counting sequence number to each
