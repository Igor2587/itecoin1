# Support for Output Descriptors in Itecoin Core

Since Itecoin Core v0.17, there is support for Output Descriptors. This is a
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.Itecoin.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
  - `xpub` encoded extended public key or `xprv` encoded extended private key (as defined in [BIP 32](https://github.com/Itecoin/bips/blob/master/bip-0032.mediawiki)).
...
- P2SH addresses (base58, of the form `3...` for mainnet or `2...` for testnet, defined in [BIP 13](https://github.com/Itecoin/bips/blob/master/bip-0013.mediawiki)).
- Segwit addresses (bech32 and bech32m, of the form `bc1...` for mainnet or `tb1...` for testnet, defined in [BIP 173](https://github.com/Itecoin/bips/blob/master/bip-0173.mediawiki) and [BIP 350](https://github.com/Itecoin/bips/blob/master/bip-0350.mediawiki)).
...
on Itecoin's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a Itecoin Core descriptor wallet to serve as your
...
the entire path from the master down. [BIP174](https://github.com/Itecoin/bips/blob/master/bip-0174.mediawiki) standardizes this by
...
existing Itecoin Core wallets, a convenience function `combo` is
...
All RPCs in Itecoin Core will include the checksum in their output. Only
