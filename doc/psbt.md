# PSBT Howto for Itecoin Core

Since Itecoin Core 0.17, an RPC interface exists for Partially Signed Itecoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/Itecoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Itecoin transactions that are not fully signed
...
#### Multisig with multiple Itecoin Core instances
...
Itecoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `Itecoin-cli` is used.
