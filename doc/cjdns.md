# CJDNS support in Itecoin Core

It is possible to run Itecoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Itecoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Itecoin network and individual nodes.
...
before it will work with your Itecoin Core node. This is described in steps
...
## Run Itecoin Core with CJDNS

Once you are connected to the CJDNS network, the following Itecoin Core
...
When enabled, this option tells Itecoin Core that it is running in an
...
IPv6 local network. This helps Itecoin Core perform better address management:
...
CJDNS support was added to Itecoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `Itecoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/Itecoin/Itecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Itecoin Core

There are several ways to see your CJDNS address in Itecoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `Itecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `Itecoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `Itecoin-cli help getnodeaddresses` for details.
